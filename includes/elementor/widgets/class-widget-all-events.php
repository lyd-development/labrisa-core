...
/**
 * Elementor "All Events" widget.
 *
 * Displays every published event (no past/upcoming filtering at all) as an
 * autoplaying, infinite-loop Swiper carousel of their event_banner_image —
 * a continuously auto-advancing "marquee" style carousel, unlike the
 * Upcoming Events widget which is manual-navigation only. Content, styling,
 * and the "Explore" popup otherwise match Upcoming Events.
 *
...
/**
 * Class Labrisa_Core_Elementor_Widget_All_Events
 *
 * @since      1.0.0
 * @package    Labrisa_Core
 * @subpackage Labrisa_Core/includes/elementor/widgets
 */
class Labrisa_Core_Elementor_Widget_All_Events extends \Elementor\Widget_Base {

	public function get_name() {
		return 'labrisa-all-events';
	}

	public function get_title() {
		return __( 'All Events', 'labrisa-core' );
	}
	...
	public function get_keywords() {
		return array( 'event', 'events', 'all events', 'carousel', 'marquee', 'autoplay', 'slider', 'popup' );
	}

	public function get_style_depends() {
		return array( 'labrisa-core-all-events' );
	}
	...
	protected function register_query_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'min'         => -1,
				'max'         => 100,
				'description' => __( 'Use -1 to show all events.', 'labrisa-core' ),
			)
		);
	...
	/**
	 * Content Tab: Swiper carousel controls, including autoplay — this is
	 * the main thing that differs from the Upcoming Events widget.
	 */
	...
	protected function render() {
		...
		if ( ! $query->have_posts() ) {
			printf(
				...
				esc_html__( 'No events found.', 'labrisa-core' )
			);
			return;
		}
	 ...
	 * by public/js/labrisa-core-all-events.js to populate the popup.
	 *
	 * @param int    $post_id
	 * @param string $title
	 ...
	 * this widget instance. public/js/labrisa-core-all-events.js fills it in
	 * and moves it to the end of <body> on first use.
	 */
